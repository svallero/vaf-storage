...
#
# af-alien-lib.sh -- by Dario Berzano <dario.berzano@cern.ch>
#
...
#
# Global variables (nothing needs to be modified, probably)
#
...
function TokenInit() {
  for ((I=0; $I < $TRIES; I++)); do
    alien-token-init $AF_ALIEN_USER > /dev/null 2>&1
    if [ $? == 0 ]; then
      source "$ALIEN_ENV"
      return 0
    fi
...
function Init() {
  ...
  export PATH="$AF_ALIEN_DIR/api/bin:$PATH"
  export LD_LIBRARY_PATH="$AF_ALIEN_DIR/api/lib:$LD_LIBRARY_PATH"
